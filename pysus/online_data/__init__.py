...
def list_data_sources() -> str:
    ...
    If the databases directory is found, it lists the supported databases.
    If the package for the databases modules is not found, it alerts that
    no support for the databases was found and lists the
        expected databases for implementation.

    Returns:
    - A formatted string listing the supported databases or the
        expected databases for implementation.
    """
...
class FTP_Inspect:
    ...
    def last_update_df(self) -> pd.DataFrame:  # Legacy
        ...
        for pth in DB_PATHS[self.database]:
            ftp.cwd(pth)
            flist = ftp.retrlines("LIST", parse)  # NOQA F841
    ...
    def list_available_years(
        ...
        bef_2000 = lambda yrs: [  # NOQA E731
            "19" + y for y in yrs if y > cur_year and y <= "99"
        ]  # NOQA F841
        aft_2000 = lambda yrs: [  # NOQA E731
        ...
        for path in self._ds_paths:
            try:
                ...
                # PNI
                    ...
                    if not PNI_group:
                        raise ValueError("No group assigned to PNI_group")
                    available_dbs.extend(ftp.nlst(f"{path}/{PNI_group}*.DBF"))
                # SINAN
                elif self.database == "SINAN":
                    if not SINAN_disease:
                        raise ValueError(
                            "No disease assigned to SINAN_disease"
                        )
